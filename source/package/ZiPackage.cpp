...
bool ZiPackageInternal::WriteBulk(const MemoryBlock& blk)
{
	...
	if ( 0 != blk.size() )
	{
		return zipWriteInFileInZip( m_zf, blk.offset(0), static_cast<unsigned int>(blk.size()) ) >= 0;
	}
	else
	{
		return true;
...
bool UnziPackageInternal::ReadBulk(MemoryBlock& blk, size_t& validLength)
{
	...
	iRead = unzReadCurrentFile( m_uf, blk.offset(0), static_cast<unsigned int>(blk.size()));
	if (iRead < 0)
	{
		return false;
	}
...
bool UnziPackageInternal::ReadBulk(MemoryBlock& blk)
{
	...
	blk.alloc( fileInfo.uncompressed_size );

	return unzReadCurrentFile( m_uf, blk.offset(0), static_cast<unsigned int>(blk.size())) == fileInfo.uncompressed_size;
}
